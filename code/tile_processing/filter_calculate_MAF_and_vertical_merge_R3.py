    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file == 'variants_unique_ann.csv':
                ...
                # Match only directories ending with _R3_1, _R3_2, or _R3_3
                match = re.match(r"(.*)(_R3_[123])$", parent_dir)
                if match:
...
for sample_name, file_list in matching_files.items():
    # Sort file list based on suffix (_R3_1, _R3_2, _R3_3) to maintain order
    ...
    for file_path, suffix in file_list:
        ...
        if suffix == '_R3_1':
            df_filtered = df[(df['protein_start'] >= 394) & (df['protein_start'] <= 429)]
        elif suffix == '_R3_2':
            df_filtered = df[(df['protein_start'] >= 430) & (df['protein_start'] <= 465)]
    ...
    output_file = f"merged_table_{sample_name}_R3.csv"
