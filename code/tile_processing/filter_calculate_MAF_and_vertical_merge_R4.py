    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file == 'variants_unique_ann.csv':
                ...
                # Match only directories ending with _R4_1, _R4_2, or _R4_3
                match = re.match(r"(.*)(_R4_[123])$", parent_dir)
                if match:
...
for sample_name, file_list in matching_files.items():
    # Sort file list based on suffix (_R4_1, _R4_2, _R4_3) to maintain order
    ...
    for file_path, suffix in file_list:
        ...
        if suffix == '_R4_1':
            df_filtered = df[(df['protein_start'] >= 466) & (df['protein_start'] <= 490)]
        elif suffix == '_R4_2':
            df_filtered = df[(df['protein_start'] >= 491) & (df['protein_start'] <= 512)]
    ...
    output_file = f"merged_table_{sample_name}_R4.csv"
