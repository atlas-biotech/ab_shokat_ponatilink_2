    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file == 'variants_unique_ann.csv':
                ...
                # Match only directories ending with _R2_1, _R2_2, or _R2_3
                match = re.match(r"(.*)(_R2_[123])$", parent_dir)
                if match:
...
for sample_name, file_list in matching_files.items():
    # Sort file list based on suffix (_R2_1, _R2_2, _R2_3) to maintain order
    ...
    for file_path, suffix in file_list:
        ...
        if suffix == '_R2_1':
            df_filtered = df[(df['protein_start'] >= 322) & (df['protein_start'] <= 357)]
        elif suffix == '_R2_2':
            df_filtered = df[(df['protein_start'] >= 358) & (df['protein_start'] <= 393)]
    ...
    output_file = f"merged_table_{sample_name}_R2.csv"
