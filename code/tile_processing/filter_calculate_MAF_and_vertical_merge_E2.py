    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file == 'variants_unique_ann.csv':
                ...
                # Match only directories ending with _E2_1, _E2_2, or _E2_3
                match = re.match(r"(.*)(_E2_[123])$", parent_dir)
                if match:
...
for sample_name, file_list in matching_files.items():
    # Sort file list based on suffix (_E2_1, _E2_2, _E2_3) to maintain order
    ...
    for file_path, suffix in file_list:
        ...
        if suffix == '_E2_1':
            df_filtered = df[(df['protein_start'] >= 128) & (df['protein_start'] <= 160)]
        elif suffix == '_E2_2':
            df_filtered = df[(df['protein_start'] >= 161) & (df['protein_start'] <= 186)]
    ...
    output_file = f"merged_table_{sample_name}_E2.csv"
