    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file == 'variants_unique_ann.csv':
                ...
                # Match only directories ending with _R1_1, _R1_2, or _R1_3
                match = re.match(r"(.*)(_R1_[123])$", parent_dir)
                if match:
...
for sample_name, file_list in matching_files.items():
    # Sort file list based on suffix (_R1_1, _R1_2, _R1_3) to maintain order
    ...
    for file_path, suffix in file_list:
        ...
        if suffix == '_R1_1':
            df_filtered = df[(df['protein_start'] >= 242) & (df['protein_start'] <= 270)]
        elif suffix == '_R1_2':
            df_filtered = df[(df['protein_start'] >= 291) & (df['protein_start'] <= 321)]
        elif suffix == '_R1_3':
            df_filtered = df[(df['protein_start'] >= 271) & (df['protein_start'] <= 290)]
    ...
    output_file = f"merged_table_{sample_name}_R1.csv"
