    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file == 'variants_unique_ann.csv':
                ...
                # Match only directories ending with _E1_1, _E1_2, or _E1_3
                match = re.match(r"(.*)(_E1_[123])$", parent_dir)
                if match:
...
for sample_name, file_list in matching_files.items():
    # Sort file list based on suffix (_E1_1, _E1_2, _E1_3) to maintain order
    ...
    for file_path, suffix in file_list:
        ...
        if suffix == '_E1_1':
            df_filtered = df[(df['protein_start'] >= 64) & (df['protein_start'] <= 95)]
        elif suffix == '_E1_2':
            df_filtered = df[(df['protein_start'] >= 96) & (df['protein_start'] <= 127)]
    ...
    output_file = f"merged_table_{sample_name}_E1.csv"
